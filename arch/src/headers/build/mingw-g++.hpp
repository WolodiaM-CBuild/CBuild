...
class MINGW_GXX : public CBuild::Toolchain {
	...
	void post_link() {
		...
		for (std::string id : this->depends) {
			...
			if (target != NULL) {
				auto out_path =
				    target->gen_out_name(".exe", ".dll");
				unsigned int end_slash =
				    out_path.find_last_of('/');
				std::string out =
				    CBUILD_BUILD_DIR + "/" + this->id + "/" +
       ...
       public:
	std::string gen_out_name(std::string executable = ".exe",
				 std::string dyn_lib = ".dll",
				 std::string stat_lib = ".a") {
		std::string base = CBUILD_BUILD_DIR + "/" + this->id + "/" +
				   CBUILD_BUILD_OUT_DIR + "/";
	...
	void call(std::vector<std::string> *args, bool force = false,
		...
		for (std::string id : this->depends) {
			...
			if (target != NULL) {
				...
				unsigned int end_slash =
				    out_path.find_last_of('/');
				unsigned int end_dot =
				    out_path.find_last_of('.');
				std::string out = out_path.substr(
				    end_slash + 4, end_dot - end_slash - 4);
		...
		CBuild::print("Now you can see compiler output",
			      CBuild::MAGENTA);
		this->build();
