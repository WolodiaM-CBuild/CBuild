	...
	for (int i = 1; i < argc; i++) {
		...
		if (tmp == std::string("-f")) {
			...
			if (i < argc) {
				...
				try {
					args->push_back_check("toolchain_id",
							      "CBuild_all");
				} catch (std::exception &e) {
				}
			}
			...
			if (i < argc) {
				tmp = argv[i];
				args->push_back("a" + std::to_string(acnt),
						tmp);
				acnt++;
			} else {
				exit(0xFF);
			...
			if (i < argc) {
				tmp = argv[i];
				args->push_back("pa" + std::to_string(pacnt),
						tmp);
				pacnt++;
			} else {
				exit(0xFF);
...
void CBuild::loop(CBuild::RType mode,
	...
	if (gen != NULL && gen->data == std::string("make")) {
		CBuild::print("Generating Makefile ...",
		...
		if (mode == CBuild::BUILD || mode == CBuild::BUILD_RUN ||
		    mode == CBuild::RUN || mode == CBuild::DEBUG ||
		    mode == CBuild::CLEAR) {
		...
		if (mode == CBuild::RUN || mode == CBuild::TASK ||
		    mode == CBuild::CLEAR || mode == CBuild::RType::REBUILD ||
		    mode == CBuild::RType::ERROR) {
			goto gen_end;
		}
		...
		CBuild::fs::create({CBUILD_COMPILE_COMMAND_JSON},
				   CBuild::fs::type::FILE);
		// Open file
		std::ofstream ccj;
