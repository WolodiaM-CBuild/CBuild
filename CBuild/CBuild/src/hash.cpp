...
struct file {
  ...
  file(std::string path) {
    ...
    this->abs_path = path;
    this->includes.clear();
    if (std::regex_search(this->abs_path, CBuild::consts::check_type)) {
      this->cpp = true;
    } else {
      this->cpp = false;
    }
 ...
 * @param included_from => std::string -> From what file included, for
 * calculating path
 ...
 *
 * @param files => std::vector<std::string> -> File list
