...
std::string sha256(const std::string str)
{
    ...
    {
        buff << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(hash[i]);
    }
    return buff.str();
}
...
bool CBuild::load_hash(std::string hash, std::string code)
{ // Load files
...
void CBuild::save_hashes(std::vector<std::string> hash_files, std::vector<std::string> code_files)
{
    // Error
