...
void CBuild::Toolchain::add_requirment(std::string task_, CBuild::stage run_stage)
{
...
{
    // Push all arguments for normal waning (-Wall, -Wextra but -Wno-comments)
...
std::string CBuild::Toolchain::gen_out_file(std::string file)
{
    ...
    std::string base = CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_BUILD_CACHE_DIR + "/";
...
std::string CBuild::Toolchain::gen_hash_file(std::string file)
{
    ...
    std::string base = CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_HASH_DIR + "/";
    // Find dot in file name
    unsigned long pos = file.find_last_of(".");
    // Cut it out
    file = file.substr(0, pos);
    ...
    if (!force_)
    {
        ...
        for (auto elem : this->targets)
        {
            ...
            if (elem.folder)
            {
                ...
                std::vector<std::string> files = CBuild::fs::dir(elem.path);
                for (auto file : files)
                {
                    // Check if hash file exists
                    if (!CBuild::fs::exists(this->gen_hash_file(file)))
                    {
                        CBuild::fs::create({this->gen_hash_file(file)}, CBuild::fs::FILE);
                        std::ofstream f(this->gen_hash_file(file));
                        f << "-";
                        f.close();
                    }
                    // Push file only if hash is different
                    if (!CBuild::load_hash(this->gen_hash_file(file), file))
                    {
                        ret.push_back(this->cmd_str(file), this->cmd_str(this->gen_out_file(file)));
                    }
                }
            }
            // If file
    ...
    else
    {
        ...
        for (auto elem : this->targets)
        {
            ...
            if (elem.folder)
            {
                ...
                {
                    // Push file
                    ret.push_back(this->cmd_str(f), this->cmd_str(this->gen_out_file(f)));
                    // Generate hash file
                    if (!CBuild::fs::exists(this->gen_hash_file(f)))
                    {
                        CBuild::fs::create({this->gen_hash_file(f)}, CBuild::fs::FILE);
                        std::ofstream fl(this->gen_hash_file(f));
                        fl << "-";
                        fl.close();
                    }
                }
            ...
            {
                // Push file
                ret.push_back(this->cmd_str(elem.path), this->cmd_str(this->gen_out_file(elem.path)));
                // Generate hash file
                if (!CBuild::fs::exists(this->gen_hash_file(elem.path)))
                {
                    CBuild::fs::create({this->gen_hash_file(elem.path)}, CBuild::fs::FILE);
                    std::ofstream f(this->gen_hash_file(elem.path));
                    f << "-";
                    f.close();
                }
            }
...
std::string CBuild::Toolchain::gen_out_name(std::string executable, std::string dyn_lib, std::string stat_lib)
{
    // Base path
...
void CBuild::Toolchain::init()
{
    ...
    if (!CBuild::fs::exists(CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_BUILD_CACHE_DIR))
    {
        CBuild::fs::create({CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_BUILD_CACHE_DIR}, CBuild::fs::DIR);
    }
    if (!CBuild::fs::exists(CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_BUILD_OUT_DIR))
    {
        CBuild::fs::create({CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_BUILD_OUT_DIR}, CBuild::fs::DIR);
    }
    if (!CBuild::fs::exists(CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_HASH_DIR))
    {
        CBuild::fs::create({CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_HASH_DIR}, CBuild::fs::DIR);
    }
}
/* Build.hpp - main */
void CBuild::Toolchain::call(std::vector<std::string> *args, bool force, bool debug)
{
    ...
    for (std::string id : this->depends)
    {
        ...
        if (target != NULL)
        {
            ...
            this->add_compile_arg("-Wl,-rpath,\"\\$ORIGIN/../../" + target->get_id() + "/" + CBUILD_BUILD_OUT_DIR + "\"");
            this->add_link_arg("-Wl,-rpath,\"\\$ORIGIN/../../" + target->get_id() + "/" + CBUILD_BUILD_OUT_DIR + "\"");
        }
    }
    // 5 steps
    if (!force)
