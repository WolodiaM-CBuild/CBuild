...
namespace CBuild
{
    ...
    public:
        ...
        {
            puts("\nArguments:");
            puts("-a <argument> -> set argument to task or toolchain");
            puts("-pa <argument> -> set argument to runned app (only for -r, -br and -d)");
            puts("--init -> init worksapce");
            puts("--help -> this message");
            puts("-m <task name> -> call task with specified id");
            puts("-b [toolchain name] -> call toolchain with specified name or all if nothing specified in build mode");
            puts("-r [toolchain name] -> call toolchain with specified name or all if nothing specified in run mode");
            ...
            puts("-gm -> generate Makefile from toolchain execution");
	        puts("-gc -> generate compile_command.json from toolchain execution");
            puts("--rebuild -> rebuild CBuild.un executable");
            ...
            puts("CBuild/cache/hash/<toolchain_name> -> hash files of toolchain with toolchain_name");
            puts("build/<toolhain_name>/objs -> object files generated by toolchain with toolchain_name name");
        }
    };
    /**
     * @brief Init task
    ...
    public:
        ...
        void call(std::vector<std::string> __attribute_maybe_unused__)
        {
            ...
            if (!CBuild::fs::exists(CBUILD_CACHE_DIR + "/" + CBUILD_COPY_CACHE_DIR))
                CBuild::fs::create({CBUILD_CACHE_DIR + "/" + CBUILD_COPY_CACHE_DIR}, CBuild::fs::DIRECTORY);
        }
    };
} // namespace CBuild
/* namesapce Registry */
