...
class line_filebuff : public CBuild::filebuff {
	...
	void save_buffer() {
       ...
       public:
	line_filebuff(std::string file, bool refresh = false,
		      bool new_file = false)
	...
	void set_str(std::string str, unsigned int pos) {
		...
		for (unsigned int i = 0; i < this->buff.size(); i++) {
			...
			if ((tmp_pos - (signed long)line.length()) < 0) {
				...
				it->clear();
				it->append(start);
				std::vector<std::string> processed_str =
				    parse_lines(str);
				it->append(processed_str.at(0));
				for (unsigned int j = 1;
				     j < processed_str.size(); j++) {
					std::advance(it, 1);
					this->buff.insert(it,
							  processed_str.at(j));
				}
...
class str_filebuff : public CBuild::filebuff {
	...
	void save_buffer() {
       ...
       public:
	str_filebuff(std::string file, bool refresh = false,
		     bool new_file = false)
	    : filebuff(file, refresh) {
		if (!new_file) {
			this->load_buffer();
